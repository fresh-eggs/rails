...
class ReportingEndpointsTest < ActiveSupport::TestCase
  ...
  def test_http_scheme_rejected
    @reporting_endpoints.endpoints = { "csp-reports": "http://not.https.pizza" }
    assert_raises ActionDispatch::ReportingEndpoints::NonTrustworthyURLError do
      @reporting_endpoints.build
    end
  end

  def test_file_scheme_accepted
    @reporting_endpoints.endpoints = { "csp-reports": "file://not.https.pizza" }

    assert_equal "csp-reports=\"file://not.https.pizza\"", @reporting_endpoints.build
  end
